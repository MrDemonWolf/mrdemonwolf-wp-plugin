...
class Test_MRDW_Push_Meta_Box extends MRDW_Push_TestCase {

	/**
	 * @var MRDW_Push_Meta_Box
	 */
	private $meta_box;

	protected function setUp(): void {
		parent::setUp();
		$this->meta_box = new MRDW_Push_Meta_Box();
	}
	...
	public function test_save_meta_box_rejects_invalid_nonce() {
		...
		$_POST = array(
			'mrdw_push_meta_box_nonce' => 'bad_nonce',
		);

		Functions\expect( 'wp_verify_nonce' )
			->with( 'bad_nonce', 'mrdw_push_meta_box' )
			->andReturn( false );
	...
	public function test_save_meta_box_saves_meta() {
		...
		$_POST = array(
			...
			'mrdw_push_custom_body'    => 'Custom Body',
		);

		Functions\expect( 'wp_verify_nonce' )
			->with( 'good_nonce', 'mrdw_push_meta_box' )
			->andReturn( true );
	...
	/**
	 * Test save_meta_box requires mrdw_manage capability.
	 */
	public function test_save_meta_box_requires_mrdw_manage() {
		$post = Mockery::mock( 'WP_Post' );
		$post->post_type = 'post';
		...
		// update_post_meta should NOT be called since mrdw_manage check fails.
		$this->meta_box->save_meta_box( 1, $post );
		$this->assertTrue( true );
	}
	...
	public function test_save_meta_box_saves_zero_when_unchecked() {
		...
		$this->assertSame( '0', $saved_meta['_mrdw_push_notify'] );
		$this->assertSame( '0', $saved_meta['_mrdw_push_include_image'] );
	}
