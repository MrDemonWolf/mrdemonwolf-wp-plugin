<?php
/**
 * Tests for MRDW_Push_Notification.
 *
 * @package MrDemonWolf
 */
...
class Test_MRDW_Push_Notification extends MRDW_Push_TestCase {
	...
	public function test_on_post_published_skips_pages() {
		...
		Functions\expect( 'apply_filters' )
			->with( 'mrdw_push_post_types', array( 'post', 'portfolio' ) )
	...
	public function test_on_post_published_skips_when_per_post_disabled() {
		...
		Functions\expect( 'get_post_meta' )
			->with( 42, '_mrdw_push_notify', true )
			->andReturn( '0' );
	...
	public function test_on_post_published_skips_already_notified() {
		...
		Functions\expect( 'get_post_meta' )
			->with( 42, '_mrdw_push_notified', true )
			->andReturn( '1' );
	...
	public function test_on_post_published_skips_no_tokens() {
		...
		Functions\expect( 'get_post_meta' )
			->with( 42, '_mrdw_push_custom_title', true )
			->andReturn( '' );

		Functions\expect( 'get_post_meta' )
			->with( 42, '_mrdw_push_custom_body', true )
			->andReturn( '' );

		Functions\expect( 'get_option' )
			->with( 'mrdw_push_default_title', Mockery::any() )
			->andReturn( 'New from {site_name}' );

		Functions\expect( 'get_option' )
			->with( 'mrdw_push_default_body', Mockery::any() )
		...
		Functions\expect( 'get_post_meta' )
			->with( 42, '_mrdw_push_include_image', true )
			->andReturn( '' );

		Functions\expect( 'get_option' )
			->with( 'mrdw_push_use_featured_image', '1' )
			->andReturn( '1' );
		...
		Functions\expect( 'update_post_meta' )
			->with( 42, '_mrdw_push_notified', '1' )
			->once();
		Functions\expect( 'delete_post_meta' )
			->with( 42, '_mrdw_push_notified' )
			->once();
	...
	public function test_build_post_notification_params_custom() {
		...
		Functions\expect( 'get_post_meta' )->andReturnUsing( function( $id, $key, $single = false ) {
			$meta = array(
				'_mrdw_push_custom_title'  => 'Custom Title',
				'_mrdw_push_custom_body'   => 'Custom Body',
				'_mrdw_push_include_image' => '0',
			);
			return $meta[ $key ] ?? '';
		} );
	...
	public function test_build_post_notification_params_with_image() {
		...
		Functions\expect( 'get_option' )->andReturnUsing( function( $key, $default = false ) {
			$options = array(
				'mrdw_push_default_title' => '{post_title}',
				'mrdw_push_default_body'  => '{post_title}',
			);
			return $options[ $key ] ?? $default;
		} );
	...
	public function test_send_notification_returns_id() {
		...
		$result = MRDW_Push_Notification::send_notification( $params, $tokens );
	...
	public function test_send_notification_returns_false_on_failure() {
		...
		$result = MRDW_Push_Notification::send_notification(
	...
	public function test_send_notification_with_post_id() {
		...
		$result = MRDW_Push_Notification::send_notification( $params, $tokens, 'post', 42 );
	...
	public function test_schedule_notification_creates_record() {
		...
		$result = MRDW_Push_Notification::schedule_notification(
	...
	public function test_cancel_scheduled() {
		...
		Functions\expect( 'wp_unschedule_event' )
			->with( 1234567890, 'mrdw_push_send_scheduled', array( 7 ) )
			->once();

		$result = MRDW_Push_Notification::cancel_scheduled( 7 );
	...
	public function test_cancel_scheduled_not_found() {
		...
		$result = MRDW_Push_Notification::cancel_scheduled( 999 );
		$this->assertFalse( $result );
	}
