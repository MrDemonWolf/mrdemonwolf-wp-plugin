...
class TailSignal_REST_Controller {
	...
	public function check_admin_permission( $request ) {
		if ( ! current_user_can( 'tailsignal_manage' ) ) {
			return new WP_Error(
				...
				__( 'You do not have permission to perform this action.', 'mrdemonwolf' ),
				array( 'status' => 403 )
			);
		}
		return true;
				...
				'validate_callback' => function ( $value ) {
					if ( ! \ExpoSDK\Utils::isExpoPushToken( $value ) ) {
						return new WP_Error(
							...
							__( 'Invalid Expo push token format.', 'mrdemonwolf' ),
							array( 'status' => 400 )
						);
					}
					return true;
				...
				'validate_callback' => function ( $value ) {
					...
					if ( null === $decoded && JSON_ERROR_NONE !== json_last_error() ) {
						return new WP_Error(
							...
							__( 'The data field must be valid JSON.', 'mrdemonwolf' ),
							array( 'status' => 400 )
						);
					}
					return true;
				...
				'validate_callback' => function ( $value ) {
					...
					if ( $timestamp <= 0 ) {
						return new WP_Error(
							...
							__( 'Invalid date format for scheduled_at.', 'mrdemonwolf' ),
							array( 'status' => 400 )
						);
					}
					if ( $timestamp <= time() ) {
						return new WP_Error(
							...
							__( 'Scheduled time must be in the future.', 'mrdemonwolf' ),
							array( 'status' => 400 )
						);
					}
					return true;
	...
	private function check_rate_limit() {
		...
		if ( (int) $count >= 30 ) {
			return new WP_Error(
				...
				__( 'Too many requests. Please try again later.', 'mrdemonwolf' ),
				array( 'status' => 429 )
			);
		}
	...
	public function register_device( $request ) {
		...
		if ( false === $device_id ) {
			return new WP_Error(
				...
				__( 'Failed to register device.', 'mrdemonwolf' ),
				array( 'status' => 500 )
			);
		}
		...
		return new WP_REST_Response(
			array(
				'success'   => true,
				'device_id' => $device_id,
				'message'   => __( 'Device registered successfully.', 'mrdemonwolf' ),
			),
			201
		);
	}
	...
	public function unregister_device( $request ) {
		...
		if ( ! $device ) {
			return new WP_Error(
				...
				__( 'Device not found.', 'mrdemonwolf' ),
				array( 'status' => 404 )
			);
		}
		...
		return new WP_REST_Response(
			array(
				'success' => true,
				'message' => __( 'Device unregistered successfully.', 'mrdemonwolf' ),
			),
			200
		);
	}
	...
	public function send_notification( $request ) {
		...
		if ( ! empty( $scheduled_at ) ) {
			...
			if ( ! $notification_id ) {
				return new WP_Error(
					...
					__( 'Failed to schedule notification.', 'mrdemonwolf' ),
					array( 'status' => 500 )
				);
			}
			...
			return new WP_REST_Response(
				array(
					...
					'message'         => __( 'Notification scheduled successfully.', 'mrdemonwolf' ),
				),
				200
			);
		}
		...
		if ( empty( $tokens ) ) {
			return new WP_Error(
				...
				__( 'No devices found for the selected target.', 'mrdemonwolf' ),
				array( 'status' => 404 )
			);
		}
		...
		if ( ! $notification_id ) {
			return new WP_Error(
				...
				__( 'Failed to send notification.', 'mrdemonwolf' ),
				array( 'status' => 500 )
			);
		}
		...
		return new WP_REST_Response(
			array(
				...
				'message'         => __( 'Notification sent successfully.', 'mrdemonwolf' ),
			),
			200
		);
	}
	...
	public function import_devices( $request ) {
		...
		if ( empty( $files['file'] ) || empty( $files['file']['tmp_name'] ) ) {
			return new WP_Error(
				'tailsignal_no_file',
				__( 'No CSV file provided.', 'mrdemonwolf' ),
				array( 'status' => 400 )
			);
		}
		...
		if ( 'csv' !== $extension ) {
			return new WP_Error(
				...
				__( 'Only CSV files are accepted.', 'mrdemonwolf' ),
				array( 'status' => 400 )
			);
		}
		...
		if ( ! $finfo ) {
			return new WP_Error(
				...
				__( 'Unable to verify file type. MIME detection unavailable.', 'mrdemonwolf' ),
				array( 'status' => 500 )
			);
		}
		...
		if ( $detected_mime && ! in_array( $detected_mime, $allowed_mimes, true ) ) {
			return new WP_Error(
				'invalid_file',
				__( 'Invalid file type.', 'mrdemonwolf' ),
				array( 'status' => 400 )
			);
		}
		...
		if ( ! $file ) {
			return new WP_Error(
				...
				__( 'Could not read the uploaded file.', 'mrdemonwolf' ),
				array( 'status' => 400 )
			);
		}
		...
		if ( ! $headers ) {
			...
			return new WP_Error(
				...
				__( 'Invalid CSV format.', 'mrdemonwolf' ),
				array( 'status' => 400 )
			);
		}
		...
		return new WP_REST_Response(
			array(
				...
				'message' => sprintf(
					...
					__( 'Import complete: %1$d new, %2$d updated, %3$d skipped.', 'mrdemonwolf' ),
					$results['new'],
					$results['updated'],
					$results['skipped']
				),
