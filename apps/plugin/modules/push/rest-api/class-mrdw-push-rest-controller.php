<?php
/**
 * REST API controller for MRDW_Push.
 *
 * @package MrDemonWolf
 */
...
class MRDW_Push_REST_Controller {
	...
	public function check_admin_permission( $request ) {
		if ( ! current_user_can( 'mrdw_manage' ) ) {
			return new WP_Error(
				'mrdw_push_forbidden',
				__( 'You do not have permission to perform this action.', 'mrdw' ),
				array( 'status' => 403 )
			);
		}
		return true;
		...
		return array(
			...
			'data'         => array(
				...
				'validate_callback' => function ( $value ) {
					...
					if ( null === $decoded && JSON_ERROR_NONE !== json_last_error() ) {
						return new WP_Error(
							'mrdw_push_invalid_json',
							__( 'The data field must be valid JSON.', 'mrdw' ),
							array( 'status' => 400 )
						);
					}
					return true;
			...
			'scheduled_at' => array(
				...
				'validate_callback' => function ( $value ) {
					...
					if ( $timestamp <= 0 ) {
						return new WP_Error(
							'mrdw_push_invalid_date',
							__( 'Invalid date format for scheduled_at.', 'mrdw' ),
							array( 'status' => 400 )
						);
					}
					if ( $timestamp <= time() ) {
						return new WP_Error(
							'mrdw_push_past_date',
							__( 'Scheduled time must be in the future.', 'mrdw' ),
							array( 'status' => 400 )
						);
					}
					return true;
	...
	private function check_rate_limit() {
		...
		if ( (int) $count >= 30 ) {
			return new WP_Error(
				'mrdw_push_rate_limited',
				__( 'Too many requests. Please try again later.', 'mrdw' ),
				array( 'status' => 429 )
			);
		}
	...
	public function register_device( $request ) {
		...
		$device_id = MRDW_Push_DB::insert_device( $data );

		if ( false === $device_id ) {
			return new WP_Error(
				'mrdw_push_registration_failed',
				__( 'Failed to register device.', 'mrdw' ),
				array( 'status' => 500 )
			);
		}
		...
		return new WP_REST_Response(
			array(
				'success'   => true,
				'device_id' => $device_id,
				'message'   => __( 'Device registered successfully.', 'mrdw' ),
			),
			201
		);
	}
	...
	public function unregister_device( $request ) {
		...
		$device = MRDW_Push_DB::get_device_by_token( $expo_token );

		if ( ! $device ) {
			return new WP_Error(
				'mrdw_push_not_found',
				__( 'Device not found.', 'mrdw' ),
				array( 'status' => 404 )
			);
		}

		if ( $device->is_active ) {
			MRDW_Push_DB::remove_device( $expo_token );
		}

		return new WP_REST_Response(
			array(
				'success' => true,
				'message' => __( 'Device unregistered successfully.', 'mrdw' ),
			),
			200
		);
	}
	...
	public function send_notification( $request ) {
		...
		if ( ! empty( $scheduled_at ) ) {
			...
			if ( ! $notification_id ) {
				return new WP_Error(
					'mrdw_push_schedule_failed',
					__( 'Failed to schedule notification.', 'mrdw' ),
					array( 'status' => 500 )
				);
			}
		...
		if ( empty( $tokens ) ) {
			return new WP_Error(
				'mrdw_push_no_devices',
				__( 'No devices found for the selected target.', 'mrdw' ),
				array( 'status' => 404 )
			);
		}
		...
		if ( ! $notification_id ) {
			return new WP_Error(
				'mrdw_push_send_failed',
				__( 'Failed to send notification.', 'mrdw' ),
				array( 'status' => 500 )
			);
		}
		...
		return new WP_REST_Response(
			array(
				...
				'message'         => __( 'Notification sent successfully.', 'mrdw' ),
			),
			200
		);
	}
	...
	public function get_stats( $request ) {
		...
		$summary = MRDW_Push_DB::get_device_summary_stats();
	...
	public function export_devices( $request ) {
		$devices = MRDW_Push_DB::get_devices_for_export();
		...
		$response->header( 'Content-Disposition', 'attachment; filename="mrdw-push-devices-' . gmdate( 'Y-m-d' ) . '.csv"' );
	...
	public function import_devices( $request ) {
		...
		if ( 'csv' !== $extension ) {
			return new WP_Error(
				'mrdw_push_invalid_file_type',
				__( 'Only CSV files are accepted.', 'mrdw' ),
				array( 'status' => 400 )
			);
		}
		...
		if ( ! $finfo ) {
			return new WP_Error(
				'mrdw_push_mime_check_failed',
				__( 'Unable to verify file type. MIME detection unavailable.', 'mrdw' ),
				array( 'status' => 500 )
			);
		}
		$detected_mime = finfo_file( $finfo, $file_info['tmp_name'] );
		if ( PHP_VERSION_ID < 80500 ) {
			// finfo_open() returns an object from PHP 8.5, where finfo_close()
			// is deprecated and closing is handled by the garbage collector.
			finfo_close( $finfo ); // phpcs:ignore Generic.PHP.DeprecatedFunctions.Deprecated -- Guarded to PHP < 8.5, where it is not deprecated.
		}
		if ( $detected_mime && ! in_array( $detected_mime, $allowed_mimes, true ) ) {
			return new WP_Error(
				'invalid_file',
				__( 'Invalid file type.', 'mrdw' ),
				array( 'status' => 400 )
			);
		}
		...
		if ( ! $file ) {
			return new WP_Error(
				'mrdw_push_file_error',
				__( 'Could not read the uploaded file.', 'mrdw' ),
				array( 'status' => 400 )
			);
		}
		...
		if ( ! $headers ) {
			fclose( $file );
			return new WP_Error(
				'mrdw_push_invalid_csv',
				__( 'Invalid CSV format.', 'mrdw' ),
				array( 'status' => 400 )
			);
		}
		...
		$results = MRDW_Push_DB::import_devices( $rows );
		...
		return new WP_REST_Response(
			array(
				...
				'message' => sprintf(
					...
					__( 'Import complete: %1$d new, %2$d updated, %3$d skipped.', 'mrdw' ),
					$results['new'],
					$results['updated'],
					$results['skipped']
				),
