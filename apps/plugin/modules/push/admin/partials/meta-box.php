...
<div class="tailsignal-meta-box">
	<?php if ( ! $notified ) : ?>
		<p>
			<label>
				...
				<?php esc_html_e( 'Send notification on publish', 'mrdemonwolf' ); ?>
			</label>
		</p>
	<?php else : ?>
		<p class="description" style="color: var(--ts-success, #2d8a4e);">
			&#10003; <?php esc_html_e( 'Auto-notification already sent for this post.', 'mrdemonwolf' ); ?>
		</p>
	<?php endif; ?>
	...
	<p>
		<label>
			...
			<?php esc_html_e( 'Include featured image', 'mrdemonwolf' ); ?>
		</label>
	</p>

	<hr />

	<h4 style="margin-bottom: 8px; font-weight: 600;"><?php esc_html_e( 'Quick Send', 'mrdemonwolf' ); ?></h4>

	<p>
		<label style="font-size: 12px; font-weight: 500; color: #374151;"><?php esc_html_e( 'Title:', 'mrdemonwolf' ); ?></label>
	...
	<p>
		<label style="font-size: 12px; font-weight: 500; color: #374151;"><?php esc_html_e( 'Body:', 'mrdemonwolf' ); ?></label>
	...
	<?php if ( $dev_mode ) : ?>
		<p class="description" style="color: var(--ts-warning, #b54708); font-size: 12px;">
			&#x26A0;&#xFE0F; <?php esc_html_e( 'Dev Mode is ON — "All devices" will only send to dev devices.', 'mrdemonwolf' ); ?>
		</p>
	<?php endif; ?>
	...
	<p>
		<button type="button" class="button tailsignal-quick-send-btn" data-post-id="<?php echo esc_attr( $post->ID ); ?>" style="background: linear-gradient(135deg, #0FACED 0%, #0991d4 100%); border-color: #0880bc; color: #fff; font-weight: 600;">
			<?php esc_html_e( 'Send Now', 'mrdemonwolf' ); ?>
		</button>
	...
	<?php if ( ! empty( $history ) ) : ?>
		<hr />
		<h4 style="margin-bottom: 4px; font-weight: 600; font-size: 12px; text-transform: uppercase; letter-spacing: 0.04em; color: #9c9690;"><?php esc_html_e( 'Send History', 'mrdemonwolf' ); ?></h4>
			...
			<?php foreach ( $history as $entry ) : ?>
				<li style="font-size: 12px; color: #6b6560; padding: 3px 0; border-bottom: 1px solid #e8e5e1;">
					...
					printf(
						...
						esc_html__( '%d devices', 'mrdemonwolf' ),
