...
class TailSignal_Devices_List_Table extends WP_List_Table {
	...
	public function get_columns() {
		return array(
			...
			'user_label'     => __( 'Label', 'mrdemonwolf' ),
			'expo_token'     => __( 'Token', 'mrdemonwolf' ),
			'device_type'    => __( 'Platform', 'mrdemonwolf' ),
			'device_model'   => __( 'Model', 'mrdemonwolf' ),
			'os_version'     => __( 'OS', 'mrdemonwolf' ),
			'app_version'    => __( 'App Ver', 'mrdemonwolf' ),
			'locale'         => __( 'Locale', 'mrdemonwolf' ),
			'last_active_at' => __( 'Last Active', 'mrdemonwolf' ),
		);
	}
	...
	public function column_user_label( $item ) {
		$label = ! empty( $item->user_label ) ? esc_html( $item->user_label ) : '<em>' . esc_html__( '(no label)', 'mrdemonwolf' ) . '</em>';
		...
		if ( ! $item->is_active ) {
			$badges .= ' <span class="tailsignal-badge tailsignal-badge-red">' . esc_html__( 'Inactive', 'mrdemonwolf' ) . '</span>';
		}
		...
		$actions = array(
			'edit'       => sprintf(
				...
				esc_html__( 'Edit Label', 'mrdemonwolf' )
			),
			'toggle_dev' => sprintf(
				'<a href="#" class="tailsignal-toggle-dev" data-id="%d" data-dev="%d">%s</a>',
				$item->id,
				$item->is_dev ? 0 : 1,
				$item->is_dev ? esc_html__( 'Remove Dev', 'mrdemonwolf' ) : esc_html__( 'Mark Dev', 'mrdemonwolf' )
			),
			'delete'     => sprintf(
				...
				esc_js( __( 'Are you sure you want to delete this device?', 'mrdemonwolf' ) ),
				esc_html__( 'Delete', 'mrdemonwolf' )
			),
		);
	...
	public function column_device_type( $item ) {
		...
		if ( empty( $item->device_type ) ) {
			return '<em>' . esc_html__( '(unknown)', 'mrdemonwolf' ) . '</em>';
		}
		...
		switch ( $column_name ) {
			...
			case 'last_active_at':
				if ( empty( $item->last_active_at ) ) {
					return '<em>' . esc_html__( 'Never', 'mrdemonwolf' ) . '</em>';
				}
				return esc_html( human_time_diff( strtotime( $item->last_active_at ), time() ) ) . ' ' . esc_html__( 'ago', 'mrdemonwolf' );
			default:
				return '';
		}
	}
	...
	public function extra_tablenav( $which ) {
		...
		echo '<option value="">' . esc_html__( 'All Platforms', 'mrdemonwolf' ) . '</option>';
		...
		echo '<option value="">' . esc_html__( 'All Devices', 'mrdemonwolf' ) . '</option>';
		echo '<option value="1"' . selected( $current_dev, '1', false ) . '>' . esc_html__( 'Dev Only', 'mrdemonwolf' ) . '</option>';
		echo '<option value="0"' . selected( $current_dev, '0', false ) . '>' . esc_html__( 'Non-Dev', 'mrdemonwolf' ) . '</option>';
		echo '</select>';

		// Group filter.
		if ( ! empty( $groups ) ) {
			echo '<select name="group_id">';
			echo '<option value="">' . esc_html__( 'All Groups', 'mrdemonwolf' ) . '</option>';
			foreach ( $groups as $group ) {
		...
		submit_button( __( 'Filter', 'mrdemonwolf' ), '', 'filter_action', false );
		echo '</div>';
	}
...
class TailSignal_Admin_Devices {
	...
	public function render() {
		...
		if ( isset( $_GET['action'] ) && 'delete' === $_GET['action'] && isset( $_GET['device_id'] ) ) {
			if ( ! current_user_can( 'tailsignal_manage' ) ) {
				wp_die( esc_html__( 'You do not have permission to perform this action.', 'mrdemonwolf' ) );
			}
	...
	public function handle_update_device() {
		...
		if ( ! $device_id ) {
			wp_send_json_error( array( 'message' => __( 'Invalid device ID.', 'mrdemonwolf' ) ) );
		}

		$result = TailSignal_DB::update_device( $device_id, array( 'user_label' => $user_label ) );

		if ( $result ) {
			wp_send_json_success( array( 'message' => __( 'Device updated.', 'mrdemonwolf' ) ) );
		} else {
			wp_send_json_error( array( 'message' => __( 'Failed to update device.', 'mrdemonwolf' ) ) );
		}
	}
	...
	public function handle_toggle_dev() {
		...
		if ( $result ) {
			wp_send_json_success(
				array(
					'message' => $is_dev
						? __( 'Device marked as dev.', 'mrdemonwolf' )
						: __( 'Device removed from dev.', 'mrdemonwolf' ),
				)
			);
		} else {
			wp_send_json_error( array( 'message' => __( 'Failed to update device.', 'mrdemonwolf' ) ) );
