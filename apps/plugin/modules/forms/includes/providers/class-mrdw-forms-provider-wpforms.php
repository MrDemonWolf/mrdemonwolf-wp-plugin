...
/**
 * Class MRDW_Forms_Provider_WPForms
 *
 * Handles WPForms integration with native notification support.
 */
class MRDW_Forms_Provider_WPForms extends MRDW_Forms_Provider {
	...
	public function create_entry( $form_id, $fields, $request ) {
		if ( ! function_exists( 'wpforms' ) || ! wpforms()->entry ) {
			return array(
				'success' => false,
				'code'    => 'entry_failed',
				'message' => __( 'WPForms is not available.', 'mrdw' ),
			);
		}
		...
		if ( ! $entry_id ) {
			return array(
				'success' => false,
				'code'    => 'entry_failed',
				'message' => __( 'Failed to save entry to WPForms.', 'mrdw' ),
			);
		}
