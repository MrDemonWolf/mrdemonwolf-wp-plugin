	...
	public function create_entry( $form_id, $fields, $request ) {
		if ( ! class_exists( 'GFAPI' ) ) {
			return array(
				'success' => false,
				'code'    => 'entry_failed',
				'message' => __( 'Gravity Forms is not available.', 'mrdemonwolf' ),
			);
		}
		...
		if ( is_wp_error( $result ) ) {
			return array(
				'success' => false,
				'code'    => 'entry_failed',
				'message' => __( 'Failed to submit form via Gravity Forms.', 'mrdemonwolf' ),
			);
		}
		...
		$message = __( 'Gravity Forms validation failed.', 'mrdemonwolf' );
