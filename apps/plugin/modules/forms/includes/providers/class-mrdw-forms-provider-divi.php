...
/**
 * Class MRDW_Forms_Provider_Divi
 *
 ...
 */
class MRDW_Forms_Provider_Divi extends MRDW_Forms_Provider {
	...
	public function create_entry( $form_id, $fields, $request ) {
		 ...
		 * @param \WP_REST_Request $request The REST request.
		 */
		$fields = apply_filters( 'mrdw_forms_pre_save_fields', $fields, $form_id, $request );
		...
		if ( ! $entry_id ) {
			return array(
				'success' => false,
				'code'    => 'entry_failed',
				'message' => __( 'Failed to save entry.', 'mrdw' ),
			);
		}
	...
	public function send_notifications( $form_id, $entry_id, $fields, $form_data ) {
		...
		if ( empty( $form_title ) ) {
			...
			$form_title = sprintf( __( 'Form %s', 'mrdw' ), $form_id );
		}
		...
		$subject = MRDW_Forms_Settings::parse_template( $subject_template, $template_data );
		$body    = nl2br( esc_html( MRDW_Forms_Settings::parse_template( $body_template, $template_data ) ) );
		 ...
		 * @param string $form_id   The form ID.
		 */
		$mail_args = apply_filters( 'mrdw_forms_notification_args', $mail_args, $entry_id, $form_id );
