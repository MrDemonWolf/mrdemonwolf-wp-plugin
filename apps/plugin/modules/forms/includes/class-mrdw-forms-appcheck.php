<?php
/**
 * Firebase App Check server-side verification.
 *
 * @package    MrDemonWolf
 * @copyright  2026 MrDemonWolf, Inc.
 */

if ( ! defined( 'ABSPATH' ) ) {
	exit;
}

/**
 * Class MRDW_Forms_AppCheck
 *
 * Verifies Firebase App Check tokens using kreait/firebase-php.
 */
class MRDW_Forms_AppCheck {

	/**
	 * Firebase Factory instance.
	 *
	 * @var \Kreait\Firebase\Factory|null
	 */
	private $factory;

	/**
	 * Constructor.
	 *
	 * @param \Kreait\Firebase\Factory|null $factory Optional factory for DI/testing.
	 */
	public function __construct( $factory = null ) {
		$this->factory = $factory;
	}

	/**
	 * Lazily create the Firebase factory.
	 *
	 * Instantiated on first verify() call only, so the kreait SDK is not
	 * loaded on every page view.
	 *
	 * @return \Kreait\Firebase\Factory
	 */
	private function get_factory() {
		if ( null === $this->factory ) {
			$this->factory = new \Kreait\Firebase\Factory();
		}

		return $this->factory;
	}

	/**
	 * Verify a Firebase App Check token.
	 *
	 * @param string $token   The App Check token from the client.
	 * @param int    $form_id The form ID.
	 * @return array{ success: bool, code?: string, message?: string, app_id?: string }
	 */
	public function verify( $token, $form_id ) {
		if ( empty( $token ) ) {
			return array(
				'success' => false,
				'code'    => 'appcheck_missing',
				'message' => __( 'App Check token is missing.', 'mrdw' ),
			);
		}

		$settings   = MRDW_Forms_Settings::get_settings();
		$project_id = $settings['firebase_project_id'] ?? '';

		if ( empty( $project_id ) ) {
			return array(
				'success' => false,
				'code'    => 'appcheck_failed',
				'message' => __( 'Firebase project ID is not configured.', 'mrdw' ),
			);
		}

		try {
			$app_check = $this->get_factory()
				->withProjectId( $project_id )
				->createAppCheck();

			$verified_token = $app_check->verifyToken( $token );
			$app_id         = $verified_token->appId;

			/**
			 * Fires after successful App Check verification.
			 *
			 * @param string $app_id  The verified app ID.
			 * @param int    $form_id The form ID.
			 * @param string $token   The original token.
			 */
			do_action( 'mrdw_forms_appcheck_verified', $app_id, $form_id, $token );

			return array(
				'success' => true,
				'app_id'  => $app_id,
			);
		} catch ( \Throwable $e ) {
			return array(
				'success' => false,
				'code'    => 'appcheck_failed',
				'message' => __( 'App Check verification failed. Please try again.', 'mrdw' ),
			);
		}
	}
}
