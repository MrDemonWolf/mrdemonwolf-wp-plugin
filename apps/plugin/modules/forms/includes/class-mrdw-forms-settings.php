...
class MRDW_Forms_Settings {
	 ...
	 * @var string
	 */
	const OPTION_NAME = 'mrdw_forms_settings';
	...
	/**
	 * Add the settings page as a submenu under MRDW_Forms.
	 */
	public function add_settings_page() {
		add_submenu_page(
			MRDW_Admin::MENU_SLUG,
			__( 'Forms Settings', 'mrdw' ),
			__( 'Forms Settings', 'mrdw' ),
			'manage_options',
			self::PAGE_SLUG,
	...
	public function register_settings() {
		...
		add_settings_field(
			...
			array(
				...
				'desc'             => __( 'Select the form builder plugin to use with MRDW_Forms.', 'mrdw' ),
			)
		);
		...
		add_settings_field(
			...
			array(
				...
				'type'  => 'text',
				'desc'  => __( 'Your Firebase project ID for App Check verification.', 'mrdw' ),
			)
		);
		...
		add_settings_field(
			...
			array(
				...
				'type'  => 'text',
				'desc'  => __( 'Comma-separated list of form IDs that MRDW_Forms can accept submissions for. For Divi, use post_id:form_index format (e.g. 42:0).', 'mrdw' ),
			)
		);

		add_settings_field(
			'allowed_origins',
			__( 'Allowed Origins', 'mrdw' ),
			array( $this, 'render_text_field' ),
			self::PAGE_SLUG,
			'mrdw_forms_general',
			array(
		...
		add_settings_field(
			...
			array(
				...
				'type'  => 'text',
				'desc'  => __( 'Subject line for notification emails sent via the REST API.', 'mrdw' ),
			)
		);
		...
		add_settings_field(
			...
			array(
				...
				'type'  => 'textarea',
				'desc'  => __( 'Body template for notification emails sent via the REST API.', 'mrdw' ),
			)
		);
	}
	...
	public function render_settings_page() {
		...
		require_once MRDW_FORMS_DIR . 'admin/settings-page.php';
	}
	...
	public function render_provider_section() {
		echo '<p>' . esc_html__( 'Choose which form builder MRDW_Forms should integrate with.', 'mrdw' ) . '</p>';
	}
	...
	public function render_appcheck_section() {
		echo '<p>' . esc_html__( 'Configure your Firebase project for App Check verification.', 'mrdw' ) . '</p>';
	}
	...
	public function render_email_template_section() {
		echo '<p>' . esc_html__( 'Customize the email notification sent for Mobile App (REST API) submissions. Divi frontend submissions use Divi\'s own email handling.', 'mrdw' ) . '</p>';
	}
	...
	public function render_text_field( $args ) {
		...
		printf(
			'<input type="%s" id="mrdw_forms_%s" name="%s[%s]" value="%s" class="regular-text" />',
			esc_attr( $type ),
			esc_attr( $field ),
	...
	public function render_notification_email_field() {
		...
		printf(
			'<input type="email" id="mrdw_forms_notification_email" name="%s[notification_email]" value="%s" class="regular-text" />',
			esc_attr( self::OPTION_NAME ),
			esc_attr( $value )
		);

		echo ' <button type="button" class="button button-secondary" id="mrdw-forms-reset-email">';
		esc_html_e( 'Reset to Admin Email', 'mrdw' );
		echo '</button>';

		echo '<p class="description">' . esc_html__( 'Where to send submission notifications. Defaults to admin email.', 'mrdw' ) . '</p>';
	}
	...
	public function render_template_field( $args ) {
		...
		if ( 'textarea' === $type ) {
			printf(
				'<textarea id="mrdw_forms_%s" name="%s[%s]" rows="6" class="large-text">%s</textarea>',
				esc_attr( $field ),
				esc_attr( self::OPTION_NAME ),
				esc_attr( $field ),
				esc_html( $value )
			...
			printf(
				'<input type="text" id="mrdw_forms_%s" name="%s[%s]" value="%s" class="regular-text" />',
				esc_attr( $field ),
				esc_attr( self::OPTION_NAME ),
				esc_attr( $field ),
				esc_attr( $value )
		...
		echo '<div class="mrdw-forms-placeholder-buttons">';
		echo '<p class="description"><strong>' . esc_html__( 'Insert variable:', 'mrdw' ) . '</strong></p>';
		foreach ( $variables as $var => $label ) {
			printf(
				'<button type="button" class="button button-small mrdw-forms-placeholder-btn" data-target="mrdw_forms_%s" data-value="%s">%s</button> ',
				esc_attr( $field ),
				esc_attr( $var ),
				esc_html( $label )
			);
	...
	public function render_select_field( $args ) {
		...
		printf(
			'<select id="mrdw_forms_%s" name="%s[%s]">',
			esc_attr( $field ),
			esc_attr( self::OPTION_NAME ),
			esc_attr( $field )
		);
		...
		foreach ( $options as $opt_value => $opt_label ) {
			...
			printf(
				...
				$disabled, // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Hard-coded attribute string built directly above.
				esc_html( $opt_label )
			);
		}
