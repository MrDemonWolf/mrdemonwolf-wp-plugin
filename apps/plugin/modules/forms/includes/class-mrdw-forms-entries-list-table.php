...
/**
 * Class MRDW_Forms_Entries_List_Table
 *
 * WP_List_Table subclass for displaying MRDW_Forms entries.
 */
class MRDW_Forms_Entries_List_Table extends \WP_List_Table {
	...
	public function get_columns() {
		return array(
			...
			'id'           => __( 'ID', 'mrdw' ),
			'source'       => __( 'Source', 'mrdw' ),
			'provider'     => __( 'Provider', 'mrdw' ),
			'form_id'      => __( 'Form ID', 'mrdw' ),
			'fields'       => __( 'Fields', 'mrdw' ),
			'ip_address'   => __( 'IP Address', 'mrdw' ),
			'date_created' => __( 'Date', 'mrdw' ),
		);
	}
	...
	public function column_id( $item ) {
		$view_url   = MRDW_Forms_Entries_Page::get_view_url( $item['id'] );
		$delete_url = wp_nonce_url(
			...
			'mrdw_forms_delete_entry_' . $item['id']
		);

		$actions = array(
			'view'   => sprintf( '<a href="%s">%s</a>', esc_url( $view_url ), __( 'View', 'mrdw' ) ),
			'delete' => sprintf( '<a href="%s" onclick="return confirm(\'%s\');">%s</a>', esc_url( $delete_url ), esc_js( __( 'Are you sure?', 'mrdw' ) ), __( 'Delete', 'mrdw' ) ),
		);
	...
	public function column_source( $item ) {
		if ( 'divi_frontend' === $item['provider'] ) {
			return '<span class="mrdw-forms-source-badge frontend">' . esc_html__( 'Divi Frontend', 'mrdw' ) . '</span>';
		}

		return '<span class="mrdw-forms-source-badge mobile">' . esc_html__( 'Mobile App', 'mrdw' ) . '</span>';
	}
	...
	public function column_provider( $item ) {
		...
		return '<span class="mrdw-forms-provider-badge">' . esc_html( $labels[ $item['provider'] ] ?? $item['provider'] ) . '</span>';
	}
	...
	protected function extra_tablenav( $which ) {
		...
		$current_provider = sanitize_text_field( wp_unslash( $_GET['provider_filter'] ?? '' ) );
		$current_source   = sanitize_text_field( wp_unslash( $_GET['source_filter'] ?? '' ) );
		?>
		<div class="alignleft actions">
			<select name="provider_filter">
				<option value=""><?php esc_html_e( 'All Providers', 'mrdw' ); ?></option>
			...
			<select name="source_filter">
				<option value=""><?php esc_html_e( 'All Sources', 'mrdw' ); ?></option>
				<option value="mobile_app" <?php selected( $current_source, 'mobile_app' ); ?>><?php esc_html_e( 'Mobile App', 'mrdw' ); ?></option>
				<option value="divi_frontend" <?php selected( $current_source, 'divi_frontend' ); ?>><?php esc_html_e( 'Divi Frontend', 'mrdw' ); ?></option>
			</select>
			<?php submit_button( __( 'Filter', 'mrdw' ), '', 'filter_action', false ); ?>
		</div>
		<?php
	}
	...
	public function prepare_items() {
		...
		if ( ! empty( $_GET['provider_filter'] ) ) {
			$args['provider'] = sanitize_text_field( wp_unslash( $_GET['provider_filter'] ) );
		}

		if ( ! empty( $_GET['source_filter'] ) ) {
			$source = sanitize_text_field( wp_unslash( $_GET['source_filter'] ) );
		...
		if ( ! empty( $_GET['form_id_filter'] ) ) {
			$args['form_id'] = sanitize_text_field( wp_unslash( $_GET['form_id_filter'] ) );
		}
	...
	public function no_items() {
		esc_html_e( 'No entries found.', 'mrdw' );
	}
}
