	...
	public function verify( $token, $form_id ) {
		if ( empty( $token ) ) {
			return array(
				...
				'message' => __( 'App Check token is missing.', 'mrdemonwolf' ),
			);
		}
		...
		if ( empty( $project_id ) ) {
			return array(
				...
				'message' => __( 'Firebase project ID is not configured.', 'mrdemonwolf' ),
			);
		}
		...
		try {
			...
			return array(
				'success' => false,
				'code'    => 'appcheck_failed',
				'message' => __( 'App Check verification failed. Please try again.', 'mrdemonwolf' ),
			);
		}
	}
}
