...
class PackRelay_Settings {
	...
	public function register_settings() {
		...
		add_settings_field(
			...
			array(
				...
				'desc'             => __( 'Select the form builder plugin to use with PackRelay.', 'mrdemonwolf' ),
			)
		);
		...
		add_settings_section(
			'packrelay_appcheck',
			__( 'Firebase App Check', 'mrdemonwolf' ),
		...
		add_settings_field(
			'firebase_project_id',
			__( 'Firebase Project ID', 'mrdemonwolf' ),
			...
			array(
				...
				'type'  => 'text',
				'desc'  => __( 'Your Firebase project ID for App Check verification.', 'mrdemonwolf' ),
			)
		);
		...
		add_settings_section(
			'packrelay_general',
			__( 'General Settings', 'mrdemonwolf' ),
			null,
			self::PAGE_SLUG
		);
		...
		add_settings_field(
			'notification_email',
			__( 'Notification Email', 'mrdemonwolf' ),
		...
		add_settings_field(
			'allowed_form_ids',
			__( 'Allowed Form IDs', 'mrdemonwolf' ),
			array( $this, 'render_text_field' ),
			self::PAGE_SLUG,
			'packrelay_general',
			array(
				...
				'type'  => 'text',
				'desc'  => __( 'Comma-separated list of form IDs that PackRelay can accept submissions for. For Divi, use post_id:form_index format (e.g. 42:0).', 'mrdemonwolf' ),
			)
		);

		add_settings_field(
			'allowed_origins',
			__( 'Allowed Origins', 'mrdemonwolf' ),
			array( $this, 'render_text_field' ),
			self::PAGE_SLUG,
			'packrelay_general',
			array(
		...
		add_settings_section(
			...
			__( 'Email Notification Template', 'mrdemonwolf' ),
		...
		add_settings_field(
			...
			array(
				...
				'type'  => 'text',
				'desc'  => __( 'Subject line for notification emails sent via the REST API.', 'mrdemonwolf' ),
			)
		);

		add_settings_field(
			'notification_body',
			__( 'Email Body', 'mrdemonwolf' ),
			array( $this, 'render_template_field' ),
			self::PAGE_SLUG,
			'packrelay_email_template',
			array(
				...
				'type'  => 'textarea',
				'desc'  => __( 'Body template for notification emails sent via the REST API.', 'mrdemonwolf' ),
			)
		);
	}
	...
	public function render_provider_section() {
		echo '<p>' . esc_html__( 'Choose which form builder PackRelay should integrate with.', 'mrdemonwolf' ) . '</p>';
	}
	...
	public function render_appcheck_section() {
		echo '<p>' . esc_html__( 'Configure your Firebase project for App Check verification.', 'mrdemonwolf' ) . '</p>';
	}
	...
	public function render_email_template_section() {
		echo '<p>' . esc_html__( 'Customize the email notification sent for Mobile App (REST API) submissions. Divi frontend submissions use Divi\'s own email handling.', 'mrdemonwolf' ) . '</p>';
	}
	...
	public function render_notification_email_field() {
		...
		esc_html_e( 'Reset to Admin Email', 'mrdemonwolf' );
		echo '</button>';

		echo '<p class="description">' . esc_html__( 'Where to send submission notifications. Defaults to admin email.', 'mrdemonwolf' ) . '</p>';
	}
	...
	public function render_template_field( $args ) {
		...
		echo '<p class="description"><strong>' . esc_html__( 'Insert variable:', 'mrdemonwolf' ) . '</strong></p>';
	...
	private function get_provider_options() {
		...
		foreach ( $providers as $slug => $data ) {
			$label = $data['label'];
			if ( ! $data['available'] ) {
				$label .= ' ' . __( '(not installed)', 'mrdemonwolf' );
			}
			$options[ $slug ] = $label;
		}
