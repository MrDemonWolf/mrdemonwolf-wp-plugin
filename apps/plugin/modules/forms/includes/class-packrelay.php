...
class PackRelay {
	...
	public function load_textdomain() {
		load_plugin_textdomain( 'mrdemonwolf', false, dirname( MRDW_PLUGIN_BASENAME ) . '/languages' );
	}
	...
	public function provider_dependency_notice() {
		if ( ! PackRelay_Activator::is_provider_available() ) {
			...
			printf(
				...
				esc_html__( 'PackRelay requires %s to be installed and active. Please install it or change the form provider in PackRelay settings.', 'mrdemonwolf' ),
				esc_html( $label )
			);
			echo '</p></div>';
		}
